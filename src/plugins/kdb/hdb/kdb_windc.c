...
krb5_error_code
ms_db_check_policy_tgs(krb5_context kcontext,
		       krb5_kdc_req *request,
		       krb5_db_entry *server,
		       krb5_ticket *ticket,
		       const char **status,
		       krb5_pa_data ***e_data)
{
    ...
    crealm = krb5_princ_realm(kdc_context, ticket->enc_part2->client);
...
#define DB_PATH	"/var/kerberos/policy.db"
	...
	*status = "dbopen() failed";
    ...
    ret = krb5_unparse_name(kdc_context, ticket->enc_part2->client,
			    &cname);
